...
def encode(article, summary, start=start, end=end, tokenizer=tokenizer, art_max_len=128, smry_max_len=50):
    ...
    art_enc = sequence.pad_sequences([tokens], padding='post', maxlen=art_max_len).squeeze()
    tokens = [start] + tokenizer.encode(summary.numpy())
    if len(tokens) > smry_max_len:
        tokens = tokens[:smry_max_len]
    else:
        tokens = tokens + [end]
    smry_enc = sequence.pad_sequences([tokens], padding='post', maxlen=smry_max_len).squeeze()
...
class Embedding(object):
    ...
    @classmethod
    def get_embedding(cls, vocab_size, embedding_dim):
        if cls.embedding is None:
            cls.embedding = tf.keras.layers.Embedding(vocab_size, embedding_dim, mask_zero=True)
...
class Decoder(tf.keras.Model):
    def __init__(self, vocab_size, embedding_dim, dec_units, batch_sz):
        ...
        self.fc1 = Dense(vocab_size, activation='softmax', name='fc1')
